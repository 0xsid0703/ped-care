import React from "react";

const Summary = () => {
  return (
    <div className="flex flex-col gap-20 py-10 text-description">
      <div className="flex flex-col mx-[350px] gap-16">
        <div className="flex flex-col gap-5 items-center">
          <div className="text-2xl font-light text-red-700">
            Pediatric Care: Ensuring Your Child&apos;s Health and Happiness
          </div>
          <div className="grid grid-cols-2 gap-10 text-center">
            <div>
              At Pediatric Care, we know how much you love your child.
              That&apos;s why we&apos;re committed to providing the best
              pediatric care in Takoma Park, MD. We work with children of all
              ages, and with a board-certified physician on staff, we&apos;re
              ready to support you and your child for a happy, healthy
              childhood.
            </div>
            <div>
              From wellness visits to sick visits, we can provide the care your
              child needs when they need it. Let Pediatric Care be here for your
              child.{" "}
              <a href="/contact" className="text-red-700">
                Contact us
              </a>{" "}
              today to schedule an appointment!
            </div>
          </div>
        </div>
        <div className="px-32 flex flex-col items-center gap-5">
          <div className="text-2xl font-light text-red-700">
            Why Choose Pediatric Care?
          </div>
          <div className="text-center">
            Choosing the right pediatrician for your child is a crucial
            decision. At Pediatric Care, we make that decision easy. Our
            experienced team, flexible appointment options, and comprehensive
            range of services ensure your child receives the best care possible.
          </div>
          <div className="text-center">
            With over 20 years of experience, our team is equipped to get the
            job done for your child. We&apos;re proud members of the American
            Academy of Pediatrics and the American Board of Pediatrics, which
            means we&apos;re committed to the highest levels of quality care.
            Our team works with children of all ages. That includes newborns,
            toddlers, grade-school children, and adolescents. We will cater our
            care to your child&apos;s unique needs.
          </div>
          <div className="text-center">
            For your convenience, we work with a wide range of insurance
            providers, and we can help streamline the process for you. Free
            parking is available at our clinic for your convenience. Overall, at
            Pediatric Care, we are proud to partner with you for your
            child&apos;s health and well-being.{" "}
            <a href="/contact" className="text-red-700">
              Contact us
            </a>{" "}
            now!
          </div>
        </div>
      </div>
      <div className="grid grid-cols-6">
        <div className="flex flex-col items-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 70 70"
            id="1866560962"
            className="text-red-700 fill-current w-[80px]"
            data-icon-name="wp-heart_beat_thin"
          >
            {" "}
            <path
              d="M49.3,10.4c-1-0.2-2.1-0.4-3.1-0.4c-5.9,0-9.6,4.1-11.2,6.4c-1.6-2.2-5.3-6.4-11.1-6.4c-1,0-2.1,0.1-3.1,0.4
	C14.4,11.9,10,18.8,10,27.3c0,2.6,0.5,5.3,1.4,8l0.9,2.2c0.5,1.2,1.2,2.5,1.9,3.6c6.6,10.5,18.4,17.5,19.7,18.3l0.7,0.4
	C34.7,60,34.8,60,35,60c0.2,0,0.3,0,0.5-0.1l0.7-0.4c1.3-0.8,13.1-7.8,19.7-18.3c0.8-1.2,1.4-2.4,2-3.8l0.8-2.2
	c0.9-2.6,1.3-5.2,1.3-7.9C60,18.8,55.6,11.9,49.3,10.4z M35.2,57.7L35,57.8l-0.2-0.1c-1.3-0.7-12.6-7.5-19-17.7
	c-0.4-0.7-0.8-1.4-1.2-2.1H26c0.4,0,0.7-0.2,0.9-0.6l1-2.1l2.2,5c0.2,0.4,0.6,0.6,1,0.6c0.4,0,0.8-0.3,0.9-0.8l2.9-11.8L38,43.2
	c0.1,0.4,0.5,0.8,0.9,0.8c0,0,0,0,0.1,0c0.4,0,0.8-0.3,0.9-0.7l3.1-9.2l1.1,3.2c0.1,0.4,0.5,0.7,0.9,0.7h10.3
	c-0.4,0.7-0.7,1.4-1.2,2.1C47.8,50.2,36.4,57,35.2,57.7z M56.8,34.5L56.2,36H45.7l-1.8-5.3C43.8,30.3,43.4,30,43,30
	s-0.8,0.3-0.9,0.7l-2.8,8.5L36,23.8c-0.1-0.5-0.5-0.8-1-0.8c-0.4,0-0.9,0.3-1,0.8l-3.3,13.1l-1.8-4.3C28.8,32.2,28.4,32,28,32
	c-0.4,0-0.7,0.2-0.9,0.6L25.4,36H13.8l-0.5-1.4c-0.8-2.5-1.2-4.9-1.2-7.3c0-7.5,3.8-13.7,9.2-15c0.9-0.2,1.8-0.3,2.6-0.3
	c6.6,0,10.2,6.7,10.3,6.8c0.2,0.3,0.5,0.5,0.9,0.5c0.4,0,0.7-0.2,0.9-0.5C36,18.4,39.5,12,46.2,12c0.9,0,1.7,0.1,2.6,0.3
	c5.4,1.3,9.2,7.4,9.2,15C58,29.7,57.6,32.1,56.8,34.5z"
            ></path>
          </svg>
          <div className="max-w-[122px] text-center text-sm">
            Care for Children of All Ages
          </div>
        </div>
        <div className="flex flex-col items-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 100 100"
            id="1594014446"
            className="text-red-700 fill-current w-[80px]"
            data-icon-name="li_award"
          >
            {" "}
            <g>
              {" "}
              <path
                d="M50,23.5c-11,0-19.9,8.9-19.9,19.9S39,63.3,50,63.3s19.9-8.9,19.9-19.9S61,23.5,50,23.5z M50,60.9
		c-9.6,0-17.5-7.8-17.5-17.5C32.5,33.8,40.4,26,50,26s17.5,7.8,17.5,17.5C67.5,53.1,59.6,60.9,50,60.9z"
              ></path>
              <path
                d="M88.3,73.7L75.6,61l2.2-1.3V51l4.4-7.6l-4.4-7.6v-8.7l-7.6-4.4l-4.4-7.6h-8.7l-7.6-4.4L42,15.2h-8.7l-4.4,7.6l-7.6,4.4v8.7
		L17,43.4l4.4,7.6v8.7l2.7,1.6L11.7,73.7c-0.4,0.4-0.5,0.9-0.3,1.4s0.6,0.8,1.2,0.8h11.2v11.2c0,0.5,0.3,1,0.8,1.2
		c0.2,0.1,0.3,0.1,0.5,0.1c0.3,0,0.6-0.1,0.9-0.4l16.2-16.2l7.5,4.3l7.6-4.4h0.6L74.2,88c0.2,0.2,0.6,0.4,0.9,0.4
		c0.2,0,0.3,0,0.5-0.1c0.5-0.2,0.8-0.6,0.8-1.2V75.8h11.2c0.5,0,1-0.3,1.2-0.8C88.7,74.6,88.6,74.1,88.3,73.7z M25.1,73.4h-9.5
		l10.7-10.7l2.6,1.5l4.4,7.6h5.4L26.3,84.1v-9.5C26.3,73.9,25.8,73.4,25.1,73.4z M56.5,69.2l-6.9,4l-6.9-4h-8l-4-6.9l-6.9-4v-8
		l-4-6.9l4-6.9v-8l6.9-4l4-6.9h8l6.9-4l6.9,4h8l4,6.9l6.9,4v8l4,6.9l-4,6.9v8l-6.9,4l-4,6.9H56.5z M74.9,73.4
		c-0.7,0-1.2,0.6-1.2,1.2v9.5L61.3,71.7h4.6l4.4-7.6l3.1-1.8l11,11L74.9,73.4L74.9,73.4z"
              ></path>
            </g>
          </svg>
          <div className="max-w-[122px] text-center text-sm">
            Care for Children of All Ages
          </div>
        </div>
        <div className="flex flex-col items-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 100 100"
            id="1975383852"
            className="text-red-700 fill-current w-[80px]"
            data-icon-name="payment_stacked_coins"
          >
            {" "}
            <path d="M76.63,47.42v-18c0-4.05-8.49-6.18-16.87-6.18s-16.87,2.12-16.87,6.18v9.68a43.09,43.09,0,0,0-9.51-1c-8.38,0-16.87,2.12-16.87,6.18V70.57c0,4.05,8.49,6.18,16.87,6.18s16.87-2.12,16.87-6.18v-3.1q1.3.3,2.76.52A16.13,16.13,0,1,0,76.63,47.42ZM45.24,32.73c3.1,1.9,8.83,2.87,14.52,2.87s11.42-1,14.52-2.87V36c0,1.3-5.13,3.83-14.52,3.83S45.24,37.3,45.24,36ZM59.76,25.6c9.4,0,14.52,2.53,14.52,3.83s-5.13,3.83-14.52,3.83-14.52-2.53-14.52-3.83S50.36,25.6,59.76,25.6ZM45.24,39.3c3.1,1.9,8.83,2.87,14.52,2.87s11.42-1,14.52-2.87v3.27c0,.4-.73,1.38-3.41,2.29A16.15,16.15,0,0,0,59.69,46.4a39.71,39.71,0,0,1-9.44-1.06V44.29c0-2-2-3.48-5-4.51Zm6.12,19.2-1.11-.26V54.33c.66.15,1.35.29,2.07.41A16.09,16.09,0,0,0,51.36,58.5Zm-.07.67c0,.23,0,.47,0,.7C51.25,59.64,51.26,59.41,51.28,59.17ZM47.9,50.86c0,1.3-5.13,3.83-14.52,3.83s-14.52-2.53-14.52-3.83V47.59c3.1,1.9,8.83,2.87,14.52,2.87s11.42-1,14.52-2.87ZM33.38,57c5.69,0,11.42-1,14.52-2.87v3.27c0,1.3-5.13,3.83-14.52,3.83s-14.52-2.53-14.52-3.83V54.16C22,56.05,27.69,57,33.38,57Zm-14.52,3.7c3.1,1.9,8.83,2.87,14.52,2.87s11.42-1,14.52-2.87V64c0,1.3-5.13,3.83-14.52,3.83S18.86,65.3,18.86,64Zm34.52-8.2a30.72,30.72,0,0,1-3.12-.61V47.76a39.59,39.59,0,0,0,6.27.88A16.11,16.11,0,0,0,53.38,52.53Zm-20-12.07c9.4,0,14.52,2.53,14.52,3.83s-5.13,3.83-14.52,3.83-14.52-2.53-14.52-3.83S24,40.46,33.38,40.46Zm0,33.94c-9.4,0-14.52-2.53-14.52-3.83V67.3c3.1,1.9,8.83,2.87,14.52,2.87s11.42-1,14.52-2.87v3.27C47.9,71.87,42.78,74.4,33.38,74.4Zm16.87-9.35V60.66l1,.21A16.07,16.07,0,0,0,52,65.43Q51.06,65.26,50.25,65.05Zm17.1,9.35A13.8,13.8,0,0,1,53.57,60.61q0-.32,0-.63A13.76,13.76,0,0,1,55,54.4a1.17,1.17,0,0,0,.08-.15A13.79,13.79,0,1,1,67.36,74.4Z"></path>
            <path d="M72.86,61.31a4.91,4.91,0,0,0-1.25-.88,9.5,9.5,0,0,0-1.44-.57c-.49-.15-1-.28-1.51-.39l-.08,0V54l.39.09a4.18,4.18,0,0,1,1.25.57,3.11,3.11,0,0,1,.83.84,3.49,3.49,0,0,1,.36.72,1.06,1.06,0,0,0,1,.7,1,1,0,0,0,.84-.42,1.05,1.05,0,0,0,.16-.94,4.79,4.79,0,0,0-.5-1.13A5,5,0,0,0,71.52,53a6.33,6.33,0,0,0-2-.9,8,8,0,0,0-1-.2V49.8a1.17,1.17,0,0,0-2.35,0v2.06a7.74,7.74,0,0,0-1.48.28,5.93,5.93,0,0,0-1.91.95,4.42,4.42,0,0,0-1.26,1.5,4.23,4.23,0,0,0-.45,1.94,4.17,4.17,0,0,0,.34,1.72,3.69,3.69,0,0,0,1,1.34,6.48,6.48,0,0,0,1.7,1,14.52,14.52,0,0,0,2,.62V67.3l-.43-.07a4.56,4.56,0,0,1-1.48-.55,3.33,3.33,0,0,1-1.06-1,3.71,3.71,0,0,1-.5-1.2A1.07,1.07,0,0,0,60.9,64a1.05,1.05,0,0,0-.23.86,5.61,5.61,0,0,0,.47,1.47,4.83,4.83,0,0,0,1.41,1.72,6.53,6.53,0,0,0,2.22,1.07,9.34,9.34,0,0,0,1.46.28v2.09a1.17,1.17,0,1,0,2.35,0v-2a7.64,7.64,0,0,0,1.66-.33,6.17,6.17,0,0,0,2-1.06,5,5,0,0,0,1.33-1.65,4.64,4.64,0,0,0,.49-2.11,4.34,4.34,0,0,0-.33-1.75A4.08,4.08,0,0,0,72.86,61.31Zm-6.62-2.39c-.37-.1-.71-.2-1-.3a4.37,4.37,0,0,1-1.18-.6,1.72,1.72,0,0,1-.56-.66,2.29,2.29,0,0,1-.17-.92,2.08,2.08,0,0,1,.92-1.8,4.21,4.21,0,0,1,2-.65Zm5.37,6.78a2.56,2.56,0,0,1-.8.89,4.08,4.08,0,0,1-1.33.59,6.21,6.21,0,0,1-.9.16V61.73l.65.18a6.92,6.92,0,0,1,1.64.69,2.05,2.05,0,0,1,.77.78,2.56,2.56,0,0,1,.23,1.15A2.45,2.45,0,0,1,71.61,65.7Z"></path>
          </svg>
          <div className="max-w-[122px] text-center text-sm">
            Most Insurance Accepted
          </div>
        </div>
        <div className="flex flex-col items-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 70 70"
            id="1018834640"
            className="text-red-700 fill-current w-[80px]"
            data-icon-name="wp-stethoscope_thin"
          >
            {" "}
            <path
              d="M53,50c-1.8,0-3.4,1-4.2,2.4c-4.9-1.7-9.8-2.9-14.8-3.7V36.9c8.3-1.1,13-16.1,13-20.9c0-3.1-1.9-5-5.2-5h-1.1
	c-0.3-0.6-1-1-1.7-1c-1.1,0-2,0.9-2,2s0.9,2,2,2c0.7,0,1.4-0.4,1.7-1h1.1c3.2,0,3.2,2.3,3.2,3c0,4.7-4.8,19-12,19
	c-7.2,0-12-14.3-12-19c0-0.7,0-3,3.2-3h1.1c0.3,0.6,1,1,1.7,1c1.1,0,2-0.9,2-2s-0.9-2-2-2c-0.7,0-1.4,0.4-1.7,1h-1.1
	c-3.2,0-5.2,1.9-5.2,5c0,4.9,4.7,19.8,13,20.9v11.6c-2.7-0.3-5.4-0.5-8-0.5c-10.8,0-12,4.2-12,6c0,3.8,4.1,6,11,6c6.4,0,11-3.8,11-9
	v-0.2c4.7,0.7,9.4,1.9,14.1,3.5c0,0.2-0.1,0.5-0.1,0.7c0,2.8,2.2,5,5,5c2.8,0,5-2.2,5-5S55.8,50,53,50z M32,51c0,4.1-3.8,7-9,7
	c-5.5,0-9-1.5-9-4c0-2.5,3.7-4,10-4c2.6,0,5.3,0.2,8,0.5V51z M53,58c-1.7,0-3-1.3-3-3s1.3-3,3-3c1.7,0,3,1.3,3,3S54.7,58,53,58z"
            ></path>
          </svg>
          <div className="max-w-[122px] text-center text-sm">
            Board-Certified Physician
          </div>
        </div>
        <div className="flex flex-col items-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 70 70"
            id="1713880920"
            className="text-red-700 fill-current w-[80px]"
            data-icon-name="wp-car_thin"
          >
            {" "}
            <g>
              {" "}
              <path
                d="M59.5,30l-3.6-12.1c-0.9-2.8-8.5-4.4-20.9-4.4c-12.3,0-19.9,1.6-20.9,4.5l-3.6,12c-0.4,0.9-0.5,1.9-0.5,2.9v14.7
		c0,1.3,0.8,2.4,2,2.8v3.2c0,1.7,1.3,3,3,3h2c1.7,0,3-1.3,3-3v-3h30v3c0,1.7,1.3,3,3,3h2c1.7,0,3-1.3,3-3v-3.2c1.2-0.4,2-1.5,2-2.8
		V32.8C60,31.8,59.8,30.8,59.5,30z M16,18.6c0.2-0.7,4.1-3.1,19-3.1s18.8,2.4,19,3l3.6,12H12.4L16,18.6z M18,53.5c0,0.6-0.4,1-1,1
		h-2c-0.6,0-1-0.4-1-1v-3h4V53.5z M55,54.5h-2c-0.6,0-1-0.4-1-1v-3h4v3C56,54.1,55.6,54.5,55,54.5z M58,47.5c0,0.6-0.4,1-1,1H13
		c-0.6,0-1-0.4-1-1V32.8c0-0.1,0-0.2,0-0.3h46c0,0.1,0,0.2,0,0.3V47.5z"
              ></path>
              <path
                d="M18,36.5c-2.2,0-4,1.8-4,4s1.8,4,4,4c2.2,0,4-1.8,4-4S20.2,36.5,18,36.5z M18,42.5c-1.1,0-2-0.9-2-2s0.9-2,2-2
		c1.1,0,2,0.9,2,2S19.1,42.5,18,42.5z"
              ></path>
              <path
                d="M52,36.5c-2.2,0-4,1.8-4,4s1.8,4,4,4c2.2,0,4-1.8,4-4S54.2,36.5,52,36.5z M52,42.5c-1.1,0-2-0.9-2-2s0.9-2,2-2
		c1.1,0,2,0.9,2,2S53.1,42.5,52,42.5z"
              ></path>
            </g>
          </svg>
          <div className="max-w-[122px] text-center text-sm">
            Free Parking for Your Appointment
          </div>
        </div>
        <div className="flex flex-col items-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 70 70"
            id="1511207141"
            className="text-red-700 fill-current w-[80px]"
            data-icon-name="wp-location_map_thin"
          >
            {" "}
            <g>
              {" "}
              <path
                d="M60,58.7l-5-18c-0.1-0.3-0.3-0.5-0.6-0.6l-9-4C45.2,36,45,36,44.8,36l-3,0.7c-0.5,0.8-0.9,1.5-1.3,2.3l3.5-0.8v6.1l-16,7.1
		l0-9.7l2.5-0.5c-0.2-0.6-0.5-1.3-0.8-1.9L27.1,40l-8.7-3.9c-0.2-0.1-0.5-0.1-0.8,0c-0.3,0.1-0.4,0.3-0.6,0.5l-7,17
		c-0.1,0.3-0.1,0.6,0,0.8c0.1,0.3,0.3,0.4,0.6,0.5l16,5c0.2,0.1,0.4,0.1,0.6,0L45,55l13.7,4.9c0.1,0,0.2,0.1,0.3,0.1
		c0.2,0,0.5-0.1,0.7-0.3C60,59.5,60.1,59.1,60,58.7z M18.5,38.3l7.5,3.3l0,8.9l-8.9-8.9L18.5,38.3z M12.3,53.4l4-9.6l9.7,9.7v4.2
		L12.3,53.4z M44,53.2l-16,4.4v-4l16-7.1V53.2z M55.5,50.1l-6-6l3.8-1.7L55.5,50.1z M46,38.5l5.5,2.5L46,43.5V38.5z M46,53.3v-7.6
		l1.5-0.7l9,9l1,3.4L46,53.3z"
              ></path>
              <path
                d="M35,45c-0.8,0-1.3-0.5-1.6-1.5l0-0.1l-0.7-2.5c-1.3-3.7-3.6-7.5-6.7-11.1c-1.9-2.2-3-5-3-7.9C23,15.3,28.4,10,35,10
		c6.6,0,12,5.3,12,11.9c0,2.9-1.1,5.7-3,7.9c-3.1,3.6-5.4,7.3-6.7,11.1l-0.7,2.5C36.3,44.5,35.8,45,35,45z M35,12
		c-5.5,0-10,4.5-10,9.9c0,2.4,0.9,4.8,2.5,6.6c3.3,3.8,5.7,7.7,7.1,11.8l0.4,1.5l0.4-1.5c1.4-4.1,3.8-8.1,7.1-11.9
		c1.6-1.8,2.5-4.2,2.5-6.6C45,16.5,40.5,12,35,12z"
              ></path>
              <path
                d="M35,27c-2.8,0-5-2.2-5-5s2.2-5,5-5c2.8,0,5,2.2,5,5S37.8,27,35,27z M35,19c-1.7,0-3,1.3-3,3s1.3,3,3,3c1.7,0,3-1.3,3-3
		S36.7,19,35,19z"
              ></path>
            </g>
          </svg>
          <div className="max-w-[122px] text-center text-sm">
            Serving Takoma Park, MD Since 2014
          </div>
        </div>
      </div>
    </div>
  );
};

export default Summary;
